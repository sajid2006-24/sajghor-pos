...
export default function Reports(){
  ...
  const profit = revenue - cost - sales.reduce((a,s)=> a+ Number(s.discount||0),0)
  ...
  const byCategory = (()=> {
    ...
    sales.forEach(s=> s.items.forEach(it=>{
      ...
      const cat = prod?.category || (it.type==='service'? t('rep.serviceCategory'): t('rep.otherCategory'))
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold">{t('rep.title')}</h1>
      ...
      {!hasData ? (
        <div className="bg-white rounded-2xl border p-10 text-center">
          ...
          <div className="mt-3 font-medium">{t('rep.noTitle')}</div>
          <div className="text-sm text-slate-500 mt-1">{t('rep.noDesc')}</div>
        </div>
      ):(
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl border p-5">
              <h3 className="font-semibold text-sm">{t('rep.byPayment')}</h3>
            ...
            <div className="bg-white rounded-2xl border p-5">
              <h3 className="font-semibold text-sm">{t('rep.byCategory')}</h3>
              {byCategory.length===0? <div className="text-sm text-slate-500 mt-3">{t('rep.noData')}</div>:
                <div className="mt-3 space-y-2">
                  {byCategory.map(c=>(
          ...
          <div className="bg-white rounded-2xl border p-5">
            <h3 className="font-semibold text-sm">{t('rep.topProducts')}</h3>
